...
class AnonMetric(ABC):
    # name - class mapping, used for when metrics should be initialized from strings
    str_alias_cls: dict[str, type[AnonMetric]] = CaseInsensitiveDict()
    ...
    @classmethod
    def from_string(cls, metric_str: str) -> AnonMetric:
    ...
    @classmethod
    def all_metrics_available(cls, return_str: bool = False) -> list[type[AnonMetric] | str]:
        return list(cls.str_alias_cls.keys()) if return_str else list(cls.str_alias_cls.values())

    @classmethod
    def metric_exists(cls, metric_cls_name: str, return_bool: bool = True) -> bool | type[AnonMetric]:
