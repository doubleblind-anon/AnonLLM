...
from src.evaluate.abstract_metric import PaddedArr, AnonMetric, Loss
...
class TestAnonMetric(unittest.TestCase):

    def test_all_metrics_available(self):

        all_cls_metrics = AnonMetric.all_metrics_available(return_str=False)
        self.assertTrue(len(all_cls_metrics) != 0)

        for cls_metric in all_cls_metrics:

            # check that instances of AnonMetric are returned
            self.assertTrue(issubclass(cls_metric, AnonMetric))
        ...
        all_str_metrics = AnonMetric.all_metrics_available(return_str=True)
    ...
    def test_from_string(self):
        ...
        with self.assertRaises(KeyError):
            AnonMetric.from_string("non-existent")
    ...
    def test_metric_exists(self):
        ...
        # test non-existing metric
        with self.assertRaises(KeyError):
            AnonMetric.metric_exists("non-existent")
            AnonMetric.metric_exists("non-existent", return_bool=True)
    ...
    def test_safe_div(self):
        ...
        result = AnonMetric.safe_div(numerator, denominator)
...
class TestLoss(unittest.TestCase):
    ...
    def test_from_string(self):
        ...
        expected = Loss()
        result = AnonMetric.from_string("loss")
        ...
        with self.assertRaises(TypeError):
            AnonMetric.from_string("loss@8")

    def test_metric_exists(self):

        # check metric_exists() works for this metric
        expected = Loss
        result = AnonMetric.metric_exists("loss", return_bool=False)
        ...
        result = AnonMetric.metric_exists("loss", return_bool=True)
        ...
        # metric
        expected = Loss
        result = AnonMetric.metric_exists("loss@invalid", return_bool=False)
